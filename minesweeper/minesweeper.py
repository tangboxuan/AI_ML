...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # mark cell as one of the moves
        self.moves_made.add(cell)
        
        # mark cell as safe
        self.mark_safe(cell)
        
        # add neighbours who are unknown to knowledge with count
        i, j = cell
        neighbours = set()
        ...
        # check knowledge to see if any safes or mines can be marked
        for sentence in self.knowledge:

            if sentence.known_mines() is not None:
                mineSet = sentence.known_mines().copy()
                for cell in mineSet:
                    self.mark_mine(cell)
            
            if sentence.known_safes() is not None:
                safeSet = sentence.known_safes().copy()
                for cell in safeSet:
                    self.mark_safe(cell)
